...
typedef unsigned long long int
    addr_t; /**<address type (unsigned long long int)*/
...
public:
  ...
  /** Initialize the graph. Build a directed graph with zero vertices and zero
   * edges. Set name of the graph with the input parameter.
  ...
  /** Delete extra vertices at the end of vertices. (No need to expose to
   * developers)
   */
   ...
   * @param dest_vertex_id - id of the destination vertex of the edge to be
   * removed
   */
   ...
   */
  type::edge_t QueryEdge(type::vertex_t src_vertex_id,
   ...
   *  Time complexity: O(A), the number of graph attributes, assuming attribute
   ...
   *  Time complexity: O(A), the number of vertex attributes, assuming attribute
   * names are not too long.
   * @param attr_name - name of the graph attribute
   * @return Logical value, TRUE if the attribute exists, FALSE otherwise.
   */
   ...
   *  Time complexity: O(A), the number of edge attributes, assuming attribute
   * names are not too long.
   * @param attr_name - name of the graph attribute
   * @return Logical value, TRUE if the attribute exists, FALSE otherwise.
   */
   ...
   */
  void SetVertexAttributeString(const char *attr_name, type::vertex_t vertex_id,
                                const char *value);
   ...
   * @param value - the (new) value of the vertex attribute
   */
  void SetVertexAttributeNum(const char *attr_name, type::vertex_t vertex_id,
   ...
   * @param value - the (new) value of the vertex attribute
   */
  void SetVertexAttributeFlag(const char *attr_name, type::vertex_t vertex_id,
                              const bool value);
   ...
   */
  void SetEdgeAttributeString(const char *attr_name, type::edge_t edge_id,
                              const char *value);
   ...
   * @param value - the (new) value of the edge attribute
   */
  void SetEdgeAttributeNum(const char *attr_name, type::edge_t edge_id,
                           const type::num_t value);
   ...
   */
  const char *GetVertexAttributeString(const char *attr_name,
   ...
   * @return value - the (new) value of the vertex attribute
   */
  const type::num_t GetVertexAttributeNum(const char *attr_name,
                                          type::vertex_t vertex_id);
   ...
   * @return value - the (new) value of the vertex attribute
   */
  const bool GetVertexAttributeFlag(const char *attr_name,
                                    type::vertex_t vertex_id);
   ...
   */
  const char *GetEdgeAttributeString(const char *attr_name,
                                     type::edge_t edge_id);
   ...
   * @return value - the (new) value of the edge attribute
   */
  const type::num_t GetEdgeAttributeNum(const char *attr_name,
                                        type::edge_t edge_id);
  ...
  /** Copy a vertex to the designated vertex. All attributes (include "id") are
   * copied.
   ...
   */
  void DeepCopyVertex(type::vertex_t new_vertex_id, Graph *g,
                      type::vertex_t vertex_id);

  /** Copy a vertex to the designated vertex. All attributes, except "id", are
   * copied.
   * @param new_vertex_id - id of the designated vertex
   * @param g - graph that contains the vertex to be copied
   * @param vertex_id - id of the vertex to be copied
   */
  void CopyVertex(type::vertex_t new_vertex_id, Graph *g,
                  type::vertex_t vertex_id);
   ...
   */
  void GetChildVertexSet(type::vertex_t vertex_id,
                         std::vector<type::vertex_t> &child_vec);
   ...
   */
  void VertexTraversal(void (*CALL_BACK_FUNC)(Graph *, type::vertex_t, void *),
                       void *extra);

  /** [Graph Algorithm] Perform Pre-order traversal on the graph.
   * @param root_vertex_id - id of the starting vertex
   * @param pre_order_vertex_vec - a vector that stores the accessing sequence
   * (id) of vertex
   */
  void PreOrderTraversal(type::vertex_t root_vertex_id,
   ...
   * @param IN_CALL_BACK_FUNC - callback function when a new vertex is
   * discovered / accessed. The input parameters of this function contain a
   * pointer to the graph being traversed, id of the accessed vertex, and an
   * extra pointer for developers to pass more parameters.
   * @param OUT_CALL_BACK_FUNC - callback function when the subtree of a vertex
   * is completed. The input parameters of this function contain a pointer to
   * the graph being traversed, id of the accessed vertex, and an extra pointer
   * for developers to pass more parameters.
   * @param extra - a pointer for developers to pass more parameters as the last
   * parameter of IN_CALL_BACK_FUNC and OUT_CALL_BACK_FUNC
   */
   ...
   * @param root - The id of the root vertex.
   * @param CALL_BACK_FUNC - callback function when a new vertex is discovered /
   * accessed. The input parameters of this function contain a pointer to the
   * graph being traversed, id of the accessed vertex, and an extra pointer for
   * developers to pass more
   * parameters.
   * @param extra - a pointer for developers to pass more parameters as the last
   * parameter of CALL_BACK_FUNC
   */
  void BFS(type::vertex_t root, void (*CALL_BACK_FUNC)(Graph *, int, void *),
           void *extra);
